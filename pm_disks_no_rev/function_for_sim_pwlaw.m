function [pos] = function_for_sim_pwlaw(L,N,v0,n,alph,D_r,...
                                    pos,theta,...
    ...
    %%%%% The quantities {v0,n,alph} determine the power law distribution
    %%%%% from which the speeds are sampled. 
    ...
    vmag_temp=v0*ones(1,N);
       ...
       for time=1:tsteps
        ...
        if(time==tot_time)
            s=rand();
            vmag_temp(pm_ind)=ret_pwlaw_nums(v_low,v_hi,alph,length(pm_ind));
            nxt_time=ceil(-log(s)./bet); %dealing in discrete time units
            tot_time=time+nxt_time;
        end
        vmag_inst=vmag_temp;
        theta(pm_ind)=theta(pm_ind)+(sqrt(2*D_r*dt)*ret_theta_gauss_dist(length(pm_ind),1));
